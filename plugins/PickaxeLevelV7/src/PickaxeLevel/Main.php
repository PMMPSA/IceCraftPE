...
class Main extends PluginBase implements Listener{
	...
	public $prefix = "§l§a♥ §e[§aIce§bCraft§e] ";
	...
	public function getNamePickaxe($player){
		...
		$this->lv->load($this->getDataFolder() . "user.yml", Config::YAML);
			$pa = "§r§l§a⚒§6 IceCraft §bᑭIᑕKᗩ᙭E§f [§cLevel: §b".$this->lv->get(strtolower($p))["Level"]."§f]§a ".$p;
		return $pa;
	}
	...
	$p->sendMessage(str_replace("{MONEY}", $money,"§l§a♥ §e[§bIce§aCraft§e] §aBạn Đã Nhận Được {MONEY} Xu Khi Đi Mine\n(Xu Này Từ Nguồn Rebirth Của Bạn)"));
		    }
		}
		if($pas[0] == "§r§l§a⚒§6"){
		 ...
		 $max = 0;
            ...
            $max = count($c);
				...
				$i = 0;
					...
					$sender->sendMessage("§¶§f-= §bTop pickaxe in §aIceCraft §b(§a".$page."§f/§a".$max."§b) §f=-");
	...
	$sender->sendMessage(str_replace("{REBIRTHS}", $amount + 1, "§l§a♥ §e[§bIce§aCraft§e] §aBạn Đã Rebirth Thành Công Và Lên Rebirth {REBIRTHS}"));
	$this->rbcoin->addRebirthCoin($sender->getName(), "10");
					}else{$sender->sendMessage("§l§a♥ §e[§bIce§aCraft§e] §aBạn Không Đủ Tiền Để Rebirth. Số Tiền Cần Để Rebirth Là: " . $price );}
		 ...
		 $max = 0;
            ...
            $max = count($c);
				...
				$i = 0;
					...
					$sender->sendMessage("§¶§f-= §bTop Chuyển Sinh Trong §aIceCraft §b(§a".$page."§f/§a".$max."§b) §f=-");
			...
			//	if(!$sender instanceof Player){
		     ...
		     $amount = $this->getRebirth($sender);
	$sender->sendMessage(str_replace("{REBIRTHS}", $amount, "§l§a♥ §e[§bIce§aCraft§e] §aBạn Đã Rebirth Lần Thứ {REBIRTHS}"));
	...
	$sender->sendMessage(str_replace("{REBIRTHS}", $amount - 1, "§l§a♥ §e[§bIce§aCraft§e] §aBạn Đã Tăng Tỉ Lệ Mine Xu Thành Công Và Tăng Tỉ Lệ Mine Xu Thành {REBIRTHS}"));
					}else{$sender->sendMessage("§l§a♥ §e[§bIce§aCraft§e] §aBạn Không Đủ Tiền Để Tăng Tỉ Lệ Mine Xu. Số Tiền Cần Để Tăng Tỉ Lệ Mine Xu Là: " . $price . " Rebirth Coin");}
                }
         return true;
     }
